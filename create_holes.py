...
class makeFiles():
    ...
    def createFiles(self,dta):
        ...
        collars = c.generate_grid_collars(grid_size, spacing, elevation, origin_x, origin_y)

        # Generate survey and assay data
        surveys, assays = c.generate_survey_and_assay_data(azimuth,dip,collars, hole_length,sampleInterval, origin_x, background_means, background_stds, high_grade_prob, high_grade_means, high_grade_stds, gradient_factor)

        # Write data to CSV files
        c.write_data_to_csv(collars, 'collars.csv', ['ID', 'X', 'Y', 'Z'])
        c.write_data_to_csv(surveys, 'surveys.csv', ['ID', 'From', 'Azimuth', 'Dip'])
        c.write_data_to_csv(assays, 'assays.csv', ['ID', 'From', 'To', 'Au', 'Cu', 'Ag'])
